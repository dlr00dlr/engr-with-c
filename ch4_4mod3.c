...
{	char isS;
	...
	printf("Enter latitude, N or S, & longitude west for location 1:\n");scanf("%lf %c %lf", &lat1, &isS, &long1);
		if (isS == 'S' || isS == 's')  lat1 *= -1;
	printf("Enter latitude, N or S, & longitude west for location 2:\n");scanf("%lf %c %lf", &lat2, &isS, &long2);
		if (isS == 'S' || isS == 's')  lat2 *= -1;
