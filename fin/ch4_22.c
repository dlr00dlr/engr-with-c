...
#include <stdlib.h>
int main(void)
{	int t=0, k=0, i=0; double speed=0, range=0, r_speed=0;
	...
	{	printf("Enter average wind speed and gust range:\n");scanf("%lf %lf",&speed,&range);
		while(t<=3600)
		{	if(rand()%200==0)for(k=0;k<=300;k+=10)
			{	if(rand()%100==0)for(i=0;i<=60;i+=10)
				{	r_speed = rand_float(speed+50-range,speed+50+range);
					fprintf(data,"%d	%f\n",t,r_speed);
					t+=10;
				}
