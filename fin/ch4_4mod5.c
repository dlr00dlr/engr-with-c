...
{	char isS, isE;
	...
	printf("Enter latitude, N or S, longitude, & W or E for location 1:\n");scanf("%lf %c %lf %c", &lat1, &isS, &long1, &isE);
		if (isS == 'S' || isS == 's')  lat1 *= -1; if (isE == 'E' || == 'e') long1 -= 360;
	printf("Enter latitude, N or S, longitude, & W or E for location 2:\n");scanf("%lf %c %lf %c", &lat2, &isS, &long2, &isE);
		if (isS == 'S' || isS == 's')  lat2 *= -1; if (isE == 'E' || == 'e') long2 -= 360;
